...
const Navbar = () => {
  ...
  return (
    <div className='fixed z-50 w-full h-20 flex items-center justify-between px-4 sm:px-15'>
        ...
        <div className={`absolute right-4 sm:right-15 z-10 min-w-35 sm:min-w-45 overflow-hidden bg-dark-textColor transition duration-300 ease-in-out ${showMenu ? 'h-70 rounded-lg top-4' : 'h-12 rounded-4xl top-[50%] bottom-[50%] translate-y-[-50%]'}`}>
          <button onClick={() => setShowMenu(!showMenu)} className={`text-dark-background w-full h-12 flex justify-between items-center font-bold text-lg px-4 ${showMenu ? 'hover:bg-dark-textColor' : 'hover:bg-dark-primary'}`}>Menu <MdMenu className='text-xl' /></button>
